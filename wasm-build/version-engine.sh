...
# 12 hex chars of the wasm's sha256 — changes iff the engine changes. sha256sum (Linux/alpine busybox)
# or shasum -a 256 (macOS) — whichever exists.
VER=$( { sha256sum openmw.wasm 2>/dev/null || shasum -a 256 openmw.wasm; } | cut -c1-12 )
[ -n "$VER" ] || { echo "version-engine: could not hash openmw.wasm" >&2; exit 1; }
