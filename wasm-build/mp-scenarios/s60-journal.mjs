...
const engineJournal = async (c) => JSON.parse(await c.eval('(window.__omwMP||{}).journalEngine||"{}"'));
const stageOf = (j, id) => {
...
export default async function run(ctx) {
  ...
  assert.equal(await b.eval('(window.__omwMP||{}).journalSent'), '0', 'B broadcast before/at sync');
  ...
  await a.waitFor(
    `Object.entries(JSON.parse((window.__omwMP||{}).journalEngine||"{}")).some(([k,v])=>k.toLowerCase()===${JSON.stringify(QUEST)}&&v===${STAGE})`,
  ...
  const sentB = await b.eval('(window.__omwMP||{}).journalSent');
  ...
  await a.eval(`Module.__omwMPCmd='quest:${QUEST}:1'`);
  await a.waitFor(
    `Object.entries(JSON.parse((window.__omwMP||{}).journalEngine||"{}")).some(([k,v])=>k.toLowerCase()===${JSON.stringify(QUEST)}&&v===1)`,
  ...
  assert.equal(await b.eval('(window.__omwMP||{}).journalSent'), '0', 'B still originated nothing');
