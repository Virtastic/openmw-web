...
export default async function run(ctx) {
  // CONTROL (temporary): no peer -- does a levitating client climb on jump at all?
  ...
  for (let i = 0; i < 8; i++) { await ctx.sleep(1000); await a.eval("if (window.omw.state) window.omw.state.body = null; 'c';"); await a.cmd('body'); await a.waitFor("typeof window.omw.state.body === 'string'", 5000, 'body'); ctx.log(`climb t+${i + 1}s: ${await a.eval('window.omw.state.body')} div=${await a.eval('window.omw.state.selfDivergence')}`); }
