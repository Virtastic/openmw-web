...
//
// Mirror semantics (scripts/mp/global.lua + player.lua): omw.state.lastChat = JSON of the last
// SERVER ChatMessage only (join/leave lines skip it), omw.state.lastChatLine = last formatted
...
export default async function run(ctx) {
  ...
  const motdExpr = `(window.omw.state.lastChat||"").includes(${JSON.stringify(ctx.motd)})`;
  ...
  // A -> B. window.omw.send queues a chatx command, drained by player.lua onFrame -> ChatSend event
  // -> server broadcast (sender included).
  const nonceA = 'n' + Math.random().toString(36).slice(2, 10);
  await a.eval(`window.omw.send('chatx:say::' + ${JSON.stringify('hello from A ' + nonceA)})`);
  let t0 = Date.now();
  await b.waitFor(`(window.omw.state.lastChat||"").includes(${JSON.stringify(nonceA)})`,
  ...
  await b.waitFor(
    `((window.omw.state.lastChatLine||"").includes(${JSON.stringify(nonceA)})
      && (window.omw.state.lastChatLine||"").includes(${JSON.stringify(a.name)}))`,
    CHAT_BUDGET_MS, `lastChatLine on ${b.name} carries A's nonce + attribution`);

  // B replies -> A.
  const nonceB = 'n' + Math.random().toString(36).slice(2, 10);
  await b.eval(`window.omw.send('chatx:say::' + ${JSON.stringify('hello back from B ' + nonceB)})`);
  t0 = Date.now();
  await a.waitFor(`(window.omw.state.lastChat||"").includes(${JSON.stringify(nonceB)})`,
    CHAT_BUDGET_MS, `B's reply on ${a.name}`);
  const dtBA = Date.now() - t0;
  await a.waitFor(
    `(window.omw.state.lastChatLine||"").includes(${JSON.stringify(nonceB)})`,
