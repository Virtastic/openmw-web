...
// lists exactly both; each client's Lua roster mirror (__omwMP.players) sees the other.
...
export default async function run(ctx) {
  ...
  // Roster propagation: each client's __omwMP.players (JSON from global.lua) includes the
  // OTHER bot within 5s (PlayerList/PlayerJoinWorld fan-out).
  const seesExpr = (other) =>
    `JSON.parse((window.__omwMP||{}).players||"[]").some(p=>p.name===${JSON.stringify(other)})`;
  ...
  const [idA, idB] = await Promise.all([a.eval('(window.__omwMP||{}).playerId'), b.eval('(window.__omwMP||{}).playerId')]);
