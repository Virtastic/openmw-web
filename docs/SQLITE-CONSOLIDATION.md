...
The locker ATTESTATION moved too (`locker_attestations`). The earlier argument for keeping it
a file — "the DMCA evidence trail should stay a `cat`" — did not hold up: the SERVER writes it
at runtime, one per account, which makes it a store like any other, exporting from SQL is
trivial, and it was inconsistent that erasure truncated that file while deleting rows
everywhere else. Erasure now DELETEs the row, because a record naming someone who asked to be
forgotten is not something to keep.

The ONLY `.json` a running server touches are operator INPUTS, never stores:
`vanilla-manifest.json` and `content-table.json`. Both are generated by tools
(`tools/gen-vanilla-manifest`, `tools/gen-content-table`) and shipped as deploy artifacts like
`config.toml` — the server only ever reads them, and putting a deploy-time input in a database
would mean you could no longer just drop in a new one.
