...
| `/opt/edge/sites/openmw-mp.caddy` | absent |
| `mp.virtastic.app` DNS | no record |
...
`mp.virtastic.app` must resolve to the VPS. The record is in `infra/terraform/main.tf`; apply
it, or add it by hand in Cloudflare. Everything else waits on this, because the OAuth redirect
...
```
https://mp.virtastic.app/auth/google/callback
https://mp.virtastic.app/auth/discord/callback
https://mp.virtastic.app/auth/microsoft/callback
```
...
## 3. `/opt/openmw-mp/data/config.toml`
...
clientId = "..."
clientSecret = "..."
redirectUri = "https://mp.virtastic.app/auth/google/callback"
