...
// .omwscripts was missing here, and nowhere else in this codebase recognised it either. It is
// how a Lua mod attaches itself to the game, which is most of the modern OpenMW ecosystem, so
// every one of them installed and then silently never loaded.
const CONTENT_EXT = /\.(esm|esp|omwaddon|omwgame|omwscripts)$/i;
// .ba2 matched in api-mods.ts but not here, so one showed up in the dashboard's archive list and
// never became a fallback-archive= line. Same set in both places now.
...
export function detectGameData(dir: string, order?: { file: string; enabled: boolean }[]): GameData {
  ...
  try {
    ...
    // withFileTypes so DIRECTORIES are skipped below. Mods live in gamedata/mods/<slug>/, and
    // without this a mod folder that happened to be called Foo.esm would be read as a plugin.
 ...
 */
export function buildPeerCfg(data: GameData, resourcesDir: string, stack?: ModStack): string {
  // ONE data= PER MOD, in the operator's order. That ordering is the whole point: OpenMW
  // resolves a loose-file conflict in favour of the LAST data= directory holding the file, so
  // the position of a mod in this list is the operator choosing which one wins. The base game
  // is first, so every mod can override it.
  //
  // content= is a SEPARATE order from data= — it decides which plugin has the final say on a
  // record rather than on a file — and masters must precede plugins. resolveMods() has already
  // sorted that out; this just lays the lines down.
  const mods = stack ?? { dataDirs: [], content: [], groundcover: [], archives: [] } as Pick<ModStack, 'dataDirs' | 'content' | 'groundcover' | 'archives'>;
  const lines = [
    '# GENERATED by openmw-mp for the Phase H simulation peer. Edits are overwritten.',
    `data=${data.dir}`,
    ...mods.dataDirs.map((d) => `data=${join(data.dir, d)}`),
    ...data.contentFiles.map((c) => `content=${c}`),
    ...mods.content.map((c) => `content=${c}`),
    // Last, matching where the browser client appends it.
    'content=mp.omwscripts',
    // Backlog 301: grass is instanced, not addressable -- tens of thousands of refs per
    // exterior the peer would otherwise simulate. groundcover= keeps it out of the content
    // list, and so out of the manifest the gate compares.
    ...(mods.groundcover ?? []).map((g) => `groundcover=${g}`),
    ...data.archives.map((a) => `fallback-archive=${a}`),
    // After the base game's: a later fallback-archive= wins, and a mod's own archive should
    // beat vanilla rather than the other way round.
    ...mods.archives.map((a) => `fallback-archive=${a}`),
