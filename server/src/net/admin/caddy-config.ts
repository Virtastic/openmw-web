...
`}
  ...
  const header = (plain: 'off' | 'no-redirects' | 'auto'): string => `# GENERATED BY THE OPENMW-WEB DASHBOARD. Edits are overwritten.
#
...

{
	...
	admin off
${plain === 'off' ? `	# No certificates and no HTTP->HTTPS redirect. Without this Caddy would try to provision
	...
	auto_https off
` : plain === 'no-redirects' ? `	# Certificates yes, HTTP->HTTPS redirect no: a fresh server's front door is plain
	# HTTP on :80, and the automatic redirect would swallow it — while https://localhost
	# keeps answering (self-signed) for a browser that remembers or insists on https.
    ...
    : domain === ''
      ? [site(':80', '\t# Plain HTTP: nobody has answered the hosting question yet, and a\n'
        + '\t# certificate warning is the wrong first thing to show an operator.'),
         // And https://localhost STILL answers, self-signed: a browser that remembers (or
         // auto-upgrades to) https://localhost would otherwise show "site can't be reached",
         // which reads as the server being down. Both doors open; http is the one we print.
         site('localhost', '\ttls internal')]
      : [site(domain, '\t# Let\'s Encrypt, automatically. No directive means a public certificate.'),
         site('localhost', '\ttls internal')];

  return `${header(internal ? 'off' : domain === '' ? 'no-redirects' : 'auto')}${blocks.join('\n')}\n`;
}
