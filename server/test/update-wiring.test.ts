...
test('compose: the server sees the client dir, and every bind source honours REPO_DIR', () => {
...
test('compose: the updater exists, holds the socket, and the web container does not', () => {
...
test('updater.sh: flag content is data, never code', () => {
...
test('updater.sh: heartbeat, status phases and expiry match what the server parses', () => {
...
test('setup scripts: tag-pinned update, REPO_DIR written, no doomed compose pull', () => {
...
test('the dashboard drives both updates through the real routes', () => {
  for (const needle of [
...
test('applying is never automatic anywhere', () => {
