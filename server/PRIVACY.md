...
| Chat log: every chat line a player typed, with timestamp, account key, display name, session playerId and channel | `logs/chat-YYYY-MM-DD.jsonl` | moderation evidence. Public play needs an answer to "what was actually said". Off with `[moderation] chatLog = false` |
